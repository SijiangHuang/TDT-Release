...
QueueDisc::Dequeue(void)
{
    ...
    /** 20191024 by sijiang
   * this assertion is disabled because in PushOut this equation is not always true
   */
...
QueueDisc::GetThreshold(void) const
{
    ...
    case QueueDiscSizePolicy::SINGLE_CHILD_QUEUE_DISC:
        if (GetNQueueDiscClasses())
        {
            return GetQueueDiscClass(0)->GetQueueDisc()->GetThreshold();
        }
    case QueueDiscSizePolicy::MULTIPLE_QUEUES:
    default:
        return m_maxSize;
...
bool QueueDisc::SetThreshold(QueueSize size)
{
    NS_LOG_FUNCTION(this << size);
    ...
    case QueueDiscSizePolicy::SINGLE_CHILD_QUEUE_DISC:
        if (GetNQueueDiscClasses())
        {
            GetQueueDiscClass(0)->GetQueueDisc()->SetThreshold(size);
        }
...
uint32_t QueueDisc::TotalBufferSize = 1000;
